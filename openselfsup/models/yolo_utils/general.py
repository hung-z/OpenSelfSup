...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False):
    ...
    if GIoU or DIoU or CIoU:
        ...
        if DIoU or CIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            # convex diagonal squared -> calculated by pythagoras
            c2 = cw ** 2 + ch ** 2 + 1e-16
...
def build_targets(p, targets, model):
    ...
    for i in range(det.nl):
        ...
        gain[2:6] = torch.tensor(p[i].shape)[[3, 2, 3, 2]]  # xyxy gain -> For turning targets xywh wrt image scales
        # to grid scales
        ...
        if nt:
            ...
            j = torch.max(r, 1. / r).max(2)[0] < model.hyp['anchor_t']  # compare -> filter all targets that is larger than anchor_t * size of anchors
            # This is because the predicted boxes are not allowed to be larger than anchor_t times of anchors
            # j = wh_iou(anchors, t[:, 4:6]) > model.hyp['iou_t']  # iou(3,n)=wh_iou(anchors(3,2), gwh(n,2))
            t = t[j]  # filter (after this all t -> 2D matrix instead of 3. The last index indicates the anchor used)
            # Offsets for expanding targets each target will be expanded to maximum of 3
            gxy = t[:, 2:4]  # grid xy
