    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                # mask1 = torch.zeros(y.shape, device=y.device, dtype=torch.bool)
                # mask2 = torch.zeros(y.shape, device=y.device, dtype=torch.bool)
                # mask1[...,0:2] = True
                # mask2[...,2:4] = True
                # y = y.masked_scatter(mask1, (y[..., 0:2] * 2. - 0.5 + self.grid[i].to(x[i].device)) * self.stride[i])
                # y = y.masked_scatter(mask2, (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i])
...
class Detect_nodefined(nn.Module):
    def __init__(self, nc=80, anchors=()):  # detection layer
        super(Detect_nodefined, self).__init__()
        self.stride = None  # strides computed during build
        self.nc = nc  # number of classes
        self.no = nc + 5  # number of outputs per anchor
        self.nl = len(anchors)  # number of detection layers
...
class Yolov4tiny(nn.Module):
    ...
    def _initialize_biases_nd_detect(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        # cf = torch.bincount(torch.tensor(np.concatenate(dataset.labels, 0)[:, 0]).long(), minlength=nc) + 1.
        m = self.model[-1]  # Detect() module
    ...
    def _print_biases_nd_detect(self):
        m = self.model[-1]  # Detect() module
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='hand/yolov4.yaml', help='model.yaml')
    ...
    for name, module in model.named_modules():
        ...
        #     print(module)
        # if isinstance(module, BottleneckCSP2):
        #     print(module.cv1.conv)
    # print(model)
